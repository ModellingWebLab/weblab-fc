...
def test_simple_ODE():
    ...
    for t in range(10):
        if t > 0:
            model.simulate(t)
        assert model.get_outputs()[model.output_names.index('a')] == a
        assert model.get_outputs()[model.output_names.index('y')] == pytest.approx(t * a)
...
def test_set_variable_with_range():
    a = 5
    model = Model.TestOdeModel(a)
