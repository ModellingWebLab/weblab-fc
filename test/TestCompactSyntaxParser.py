...
class TestCompactSyntaxParser(unittest.TestCase):
    ...
    def assertHasLocalName(self, xmlElement, localName):
        ...
        self.assertEqual(xmlElement.tag[-len(ending):], ending,
                         '%s does not have local name %s' % (xmlElement.tag, localName))

    def check_xml(self, actualXml, expectedXml, checkEmpty=True):
        if isinstance(expectedXml, str):
            ...
            if expectedXml.startswith('csymbol-'):
                ...
                expectedXml = ('csymbol',
                               {'definitionURL': 'https://chaste.cs.ox.ac.uk/nss/protocol/' + expectedXml[8:]})
            else:
                if checkEmpty:
                    self.assertEqual(len(actualXml), 0, '%s has unexpected children' % x2_s(actualXml))
                self.assertHasLocalName(actualXml, expectedXml)
        if isinstance(expectedXml, tuple):
            self.check_xml(actualXml, expectedXml[0], checkEmpty=False)
            for children_or_attrs in expectedXml[1:]:
                if isinstance(children_or_attrs, list):  # Child elements
                    self.assertEqual(len(actualXml), len(children_or_attrs),
                                     '%s != %s' % (x2_s(actualXml), children_or_attrs))
                    for i, expected_child in enumerate(children_or_attrs):
                        self.check_xml(actualXml[i], expected_child)
                    ...
                    self.assertEqual(len(actual_attrs), len(children_or_attrs), '%s != %s' %
                                     (actual_attrs, children_or_attrs))
...
#            print x2_s(actual_xml)
            self.check_xml(actual_xml, expectedXml)
        self.check_parse_results(actual_results, results)

    def fail_if_parses(self, grammar, input):
    ...
    def assert_files_match(self, newFilePath, refFilePath):
    ...
    def test_parsing_identifiers(self):
        ...
        self.fail_if_parses(csp.ncIdent, 'abc:def')
        self.fail_if_parses(csp.ncIdent, '123')
        ...
        self.fail_if_parses(csp.ident, '123')
    ...
    def test_parsing_numbers(self):
        ...
        self.fail_if_parses(csp.number, '.123')
        self.fail_if_parses(csp.number, '123.')
        self.fail_if_parses(csp.number, '+123')
        self.fail_if_parses(csp.number, '1E3')
    ...
    def test_parsing_numbers_with_units(self):
        ...
        self.assertParses(csp.expr, '123 :: dimensionless', ['123'], with_units('cn:123', 'dimensionless'))
        self.assertParses(csp.expr, '4.6e5::mA', ['4.6e5'], with_units('cn:4.6e5', 'mA'))
        self.fail_if_parses(csp.number, '123 :: ')
        self.fail_if_parses(csp.number, '123 :: prefix:units')

    def test_parsing_comments(self):
        self.assertParses(csp.comment, '# blah blah', [])
        self.fail_if_parses(csp.comment, '# blah blah\n')
    ...
    def test_parsing_simple_assignments(self):
        ...
        self.fail_if_parses(csp.simpleAssign, 'pre:var = value')
    ...
    def test_parsing_namespaces(self):
        ...
        self.fail_if_parses(csp.nsDecls, 'namespace n="uri"\n')
...
""", [[[['label_duration', '300'], ['unlabel_time', '600000'], ['use_tapasin', '1']]]])
    ...
    def test_parsing_imports(self):
        ...
        self.fail_if_parses(csp.imports, 'import "file"\n')
    ...
    timecourse_duration = 2000
        ...
        self.fail_if_parses(csp.importStmt, 'import "file.txt" { } \n')
    ...
    def test_parsing_model_interface(self):
        ...
        self.fail_if_parses(csp.inputVariable, 'input no_prefix')
        ...
        self.fail_if_parses(csp.outputVariable, 'output no_prefix')
        ...
        self.assertParses(csp.optionalVariable, 'optional p:n default p:v + local * 2 :: u',
                          ...
                          ('specifyOptionalVariable', {'name': 'p:n'},
                           [('apply', ['plus', 'ci:p:v', ('apply', ['times', 'ci:local', with_units('cn:2', 'u')])])]))
        self.fail_if_parses(csp.optionalVariable, 'optional no_prefix')
        ...
        self.fail_if_parses(csp.newVariable, 'var prefix:varname units uname = 0')
        self.fail_if_parses(csp.newVariable, 'var varname = 0')
        self.fail_if_parses(csp.newVariable, 'var varname')
        ...
        self.assertParses(csp.clampVariable, 'clamp p:v to 0 :: U', [['p:v', '0']],
                          ('addOrReplaceEquation', [('apply', ['eq', 'ci:p:v', with_units('cn:0', 'U')])]))
        ...
        self.assertParses(csp.modelEquation, 'define model:var = 2.5 :: units / local_var',
                          ...
                          ('addOrReplaceEquation', [('apply', ['eq', 'ci:model:var',
                                                               ('apply', ['divide', with_units('cn:2.5', 'units'),
                                                                          'ci:local_var'])])]))
    ...
    convert u1 to u2 by lambda u: u * test:v3
            ...
            ('modelInterface', [('setIndependentVariableUnits', {'units': 't'}),
                                ...
                                ('declareNewVariable', {'name': 'early_local',
                                                        'units': 'u', 'initial_value': '12.34'}),
                                ...
                                ('declareNewVariable', {'name': 'local',
                                                        'units': 'dimensionless', 'initial_value': '5'}),
                                ('addOrReplaceEquation', [
                                    ('apply', ['eq', 'ci:test:v3', ('apply', ['times', 'ci:test:v2', 'ci:local'])])]),
    ...
    def test_parsing_uniform_range(self):
        ...
        self.assertParses(csp.range, 'range t units s uniform start : step : end',
                          [['t', 's', ['start', 'step', 'end']]],
                          ('uniformStepper', {'name': 't', 'units': 's'},
        ...
        self.fail_if_parses(csp.range, 'range t units s uniform start:step:end')
    ...
    def test_parsing_modifiers(self):
        ...
        self.fail_if_parses(csp.saveState, 'save as state:name')

        self.assertParses(csp.resetState, 'reset', [])
        self.assertParses(csp.resetState, 'reset to state_name', ['state_name'])
        self.fail_if_parses(csp.resetState, 'reset to state:name')
        ...
        at end save as savedState
                          ...
                          ('modifiers', [('resetState', ['when:AT_START_ONLY']),
                                         ('setVariable', ['when:EVERY_LOOP',
                                                          'name:model:input', ('value', ['ci:loopVariable'])]),
    ...
    def test_parsing_timecourse_simulations(self):
        self.assertParses(csp.simulation, 'simulation sim = timecourse { range time units ms uniform 1:10 }',
                          ...
                          ('timecourseSimulation', {'prefix': 'sim'},
                           [('uniformStepper', {'name': 'time', 'units': 'ms'},
                             [('start', ['cn:1']), ('stop', ['cn:10']), ('step', ['cn:1'])]), 'modifiers']))
...
modifiers { at end save as prelim }
                          ...
                          ('timecourseSimulation', {'prefix': 'sim'},
                           [('whileStepper', {'name': 'time', 'units': 'U'},
                             [('condition', [('apply', ['lt', 'ci:time', 'cn:100'])])]),
                            ('modifiers', [('saveState', ['when:AT_END', 'name:prelim'])])]))
        self.fail_if_parses(csp.simulation, 'simulation sim = timecourse {}')
    ...
    def test_parsing_one_step_simulations(self):
        ...
        self.assertParses(csp.simulation, 'simulation oneStep { modifiers { at start set a = 1 } }',
                          [['', [[['start', ['a', '1']]]]]],
                          ('oneStep', {},
                           [('modifiers', [('setVariable', ['when:AT_START_ONLY', 'name:a', ('value', ['cn:1'])])])]))
                           ...
                           [('uniformStepper', [('start', ['cn:1']), ('stop', ['cn:2']), ('step', ['cn:1'])]),
                            ...
                            ('timecourseSimulation', {},
                             [('uniformStepper', [('start', ['cn:1']), ('stop', ['cn:100']), ('step', ['cn:1'])]),
                              'modifiers'])]))
        self.fail_if_parses(csp.simulation, 'simulation rpt = nested { range run units U while 1 }')
...
""", [[['', [['time', 'second', ['1', '1000']]]],
            ...
            ('simulations', [('timecourseSimulation', {}, [('uniformStepper',), 'modifiers']),
                             ('nestedSimulation', {'prefix': 'main'},
                              [('vectorStepper',), 'modifiers', ('subTask',)])]))
    ...
    def test_parsing_output_specifications(self):
        ...
        self.assertParses(csp.outputSpec, r'name = ref:var units U "Description \"quotes\""',
                          ...
                          ('postprocessed', {'name': 'name', 'ref': 'ref:var', 'units': 'U',
        ...
        self.assertParses(csp.outputSpec, "name = ref:var units U 'Description \\'quotes\\' \"too\"'",
                          ...
                          ('postprocessed', {'name': 'name', 'ref': 'ref:var', 'units': 'U',
                                             'description': 'Description \'quotes\' "too"'}))
        ...
        self.fail_if_parses(csp.outputSpec, 'varname_no_units')
...
""", [[['n1', 'n2', 'u1'], ['n3', 'p:m', 'd1'], ['n4', 'u2', 'd2'], ['optional', 'n5', 'u3']]],
    ...
    def test_parsing_plot_specifications(self):
        ...
        self.fail_if_parses(csp.plotCurve, 'm:y against x')
        self.fail_if_parses(csp.plotCurve, 'y against m:x')
        ...
        self.fail_if_parses(csp.plotSpec, 'plot "only title" {}')
        self.fail_if_parses(csp.plotSpec, 'plot "only title"')
    ...
    def test_parsing_function_calls(self):
        ...
        self.fail_if_parses(csp.functionCall, 'spaced (param)')
    ...
    def test_parsing_assign_statements(self):
        ...
        self.fail_if_parses(csp.assignStmt, 'pre:var = value')
        ...
        self.assertParses(csp.assignStmt, 'a, b = b, a', [[['a', 'b'], ['b', 'a']]],
                          ('apply', ['eq', ('apply', ['csymbol-tuple', 'ci:a', 'ci:b']),
                                     ('apply', ['csymbol-tuple', 'ci:b', 'ci:a'])]))
        self.assertParses(csp.assignStmt, 'a, b = (b, a)', [[['a', 'b'], [['b', 'a']]]],
                          ('apply', ['eq', ('apply', ['csymbol-tuple', 'ci', 'ci']),
                                     ('apply', ['csymbol-tuple', 'ci', 'ci'])]))
        self.fail_if_parses(csp.assignStmt, 'p:a, p:b = e')
        self.fail_if_parses(csp.assignStmt, '')
    ...
    def test_parsing_optional_assignments(self):
        ...
        self.assertParses(csp.assignStmt, 'optional = expr',
                          [[['optional'], ['expr']]], ('apply', {}, ['eq', 'ci:optional', 'ci:expr']))
    ...
    def test_parsing_assert_statements(self):
        ...
        self.assertParses(csp.assertStmt, 'assert 1', [['1']],
                          ('apply', [('csymbol',
                                      {'definitionURL': 'https://chaste.cs.ox.ac.uk/nss/protocol/assert'}), 'cn']))
...
return c, d""", [[[['a', '<', '0']],
                  ...
                  [['c', 'd'], [['a', '*', '2'], ['b', '+', '1']]],
                  ['c', 'd']]])
        self.fail_if_parses(csp.stmtList, '')
    ...
    def test_parsing_lambda_expressions(self):
        ...
        self.fail_if_parses(csp.lambdaExpr, 'lambda p:a: 5')
...
""", [[[['a'], ['b']], [[['a', '>', 'b']],
        ...
        self.assertParses(csp.expr, "lambda a { return a }", [[[['a']], [['a']]]],
                          ('lambda', [('bvar', ['ci']),
                                      ('apply', ['csymbol-statementList', ('apply', ['csymbol', 'ci'])])]))
        ...
        self.assertParses(csp.functionDefn, """def outer()
{
    ...
    def test_parsing_tuples(self):
        ...
        self.fail_if_parses(csp.tuple, '(1)')  # You need a Python-style comma as above
        self.assertParses(csp.expr, '(1,2)', [['1', '2']], ('apply', ['csymbol', 'cn', 'cn']))
        self.assertParses(csp.expr, '(1,a,3,c)', [['1', 'a', '3', 'c']],
                          ('apply', ['csymbol-tuple', 'cn', 'ci', 'cn', 'ci']))
    ...
    def test_parsing_array_comprehensions(self):
        self.assertParses(csp.array, '[i for i in 0:n]', [['i', ['i', ['0', 'n']]]],
                          ('apply', ['csymbol-newArray',
                                     ('domainofapplication', [
                                      ('apply', ['csymbol-tuple', 'cn:0', 'cn:1', 'ci:n', 'csymbol-string:i'])]),
                                     'ci:i']))
        ...
        self.assertParses(csp.array, '[i^j for i in 1:3 for 2$j in 4:-1:2]',
                          ...
                          ('apply', ['csymbol-newArray',
                                     ('domainofapplication',
                                      [('apply', ['csymbol-tuple', 'cn:1', 'cn:1', 'cn:3', 'csymbol-string:i']),
                                       ('apply', ['csymbol-tuple', 'cn:2', 'cn:4',
                                                  ('apply', ['minus', 'cn:1']), 'cn:2', 'csymbol-string:j'])]),
                                     ('apply', ['power', 'ci:i', 'ci:j'])]))
        # Dimension specifiers can be expressions too...
        self.assertParses(csp.expr, '[i for (1+2)$i in 2:(3+5)]',
                          [['i', [['1', '+', '2'], 'i', ['2', ['3', '+', '5']]]]])
        self.assertParses(csp.expr, '[i for 1+2$i in 2:4]', [['i', [['1', '+', '2'], 'i', ['2', '4']]]],
                          ('apply', ['csymbol-newArray',
                                     ('domainofapplication',
                                      [('apply', ['csymbol-tuple', ('apply', ['plus', 'cn:1', 'cn:2']),
                                                  'cn:2', 'cn:1', 'cn:4', 'csymbol-string:i'])]),
                                     'ci:i']))
        self.fail_if_parses(csp.expr, '[i for 1 $i in 2:4]')
    ...
    def test_parsing_views(self):
        ...
        self.assertParses(csp.expr, 'arr[:-alpha]', [['arr', ['', ['-', 'alpha']]]],
                          ('apply', ['csymbol-view', 'ci:arr',
                                     ('apply', ['csymbol-tuple', 'cn:0', 'csymbol-null',
                                                'cn:1', ('apply', ['minus', 'ci:alpha'])]),
                                     ('apply', ['csymbol-tuple', 'csymbol-null', 'csymbol-null', 'cn:1', 'csymbol-null'])]))
        ...
        self.fail_if_parses(csp.expr, 'arr [1]')
        self.fail_if_parses(csp.expr, 'arr[1] [3]')
        self.fail_if_parses(csp.expr, 'arr[1 $ 2]')
    ...
    def test_parsing_find_and_index(self):
        ...
        self.fail_if_parses(csp.expr, 'find (arr)')
        # self.fail_if_parses(csp.expr, 'find(arr, extra)') # Needs special support e.g. from parse actions
        ...
        self.fail_if_parses(csp.expr, 'arr {spaced}')
...
""", [[['nM', ['nano', 'mole'], ['litre', '-1']],
    ...
    def test_parsing_accessors(self):
        ...
        self.fail_if_parses(csp.expr, 'arr .SHAPE')
    ...
    def test_parsing_map(self):
        ...
        # self.fail_if_parses(csp.expr, 'map(f)') # At present implemented just as a function call with special name
    ...
    def test_parsing_fold(self):
        ...
        # self.fail_if_parses(csp, expr, 'fold()')
        # self.fail_if_parses(csp, expr, 'fold(f, A, i, d, extra)')
    ...
    def test_parsing_wrapped_mathml_operators(self):
        ...
        # self.fail_if_parses(csp.expr, '@0:+') # Best done at parse action level?
        self.fail_if_parses(csp.expr, '@1:non_mathml')
        self.fail_if_parses(csp.expr, '@ 2:*')
        self.fail_if_parses(csp.expr, '@2 :-')
        self.fail_if_parses(csp.expr, '@1:--')
        self.fail_if_parses(csp.expr, '@n:+')
