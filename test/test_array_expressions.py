...
def test_views():
    ...
    assert view.evaluate({}).array.ndim == 2
    np.testing.assert_array_almost_equal(view.evaluate({}).array, predicted)
    ...
    assert view.evaluate({}).array.ndim == 3
    np.testing.assert_array_almost_equal(view.evaluate({}).array, predicted)
...
def test_array_creation_protocol_errors():
    array = E.NewArray(E.NewArray(E.NewArray(E.N(-2), E.N(-1), E.N(0)),
                                  E.NewArray(E.N(1), E.N(2), E.N(3))),
                       E.NewArray(E.NewArray(E.N(4), E.N(5), E.N(6)),
                                  E.NewArray(E.N(1), E.N(2), E.N(3))))

    # more tuple slices than dimensions
    ...
    # too many arguments for a slice
...
def test_array_expression_protocol_errors():
    env = Env.Environment()
...
def test_map_with_multi_dimensional_arrays():
    env = Env.Environment()
    parameters = ['a', 'b', 'c']
    body = [S.Return(E.Plus(E.NameLookUp('a'), E.NameLookUp('b'), E.NameLookUp('c')))]
    add = E.LambdaExpression(parameters, body)
...
def test_using_many_operations_in_function():
    env = Env.Environment()
    parameters = ['a', 'b', 'c']
...
def test_map_with_function_with_defaults():
    env = Env.Environment()
...
def test_nested_function():
    ...
    assert result.value == predicted


def test_compile_methods_for_math_expression():
    # Minus
    env = Env.Environment()
    parameters = ['a', 'b']
...
def test_compile_methods_for_logical_expressions():
    # And
    env = Env.Environment()
    parameters = ['a', 'b']
...
def test_fold_with_different_functions():
...
def test_index_protocol_errors():
