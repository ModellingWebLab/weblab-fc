...
def test_generate_weblab_model(tmp_path):
    ...
    # Load cellml model
    model = os.path.join('test', 'models', 'real', 'hodgkin_huxley_squid_axon_model_1952_modified.cellml')
    ...
    generated_info = (tmp_path / 'model-debug-info.txt').read_text()

    # Files containing reference output
    expected = os.path.join('test', 'output', 'code_generation', 'weblab_model.pyx')
    expected_info = os.path.join('test', 'output', 'code_generation', 'model-debug-info.txt')
    ...
    if os.environ.get('WEBLAB_REGENERATE_REF'):
        ...
        with open(expected, 'w') as f:
            f.write(generated)
        with open(expected_info, 'w') as f:
            f.write(generated_info)

    # Read reference output
    with open(expected, 'r') as f:
        expected = f.read()
    with open(expected_info, 'r') as f:
    ...
    # And info should have been written
