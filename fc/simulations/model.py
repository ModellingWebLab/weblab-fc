...
class AbstractOdeModel(AbstractModel):
    ...
    def reset_state(self, name=None):
        ...
        if name is None:
            self.solver.ResetSolver(self.initial_state.copy())
        else:
            # TODO: Raise a nice ProtocolError if state not defined
            self.solver.ResetSolver(self.saved_states[name].copy())

    def simulate(self, endPoint):
        """simulate the model up to the given end point (value of the free variable)."""
        self.solver.simulate(endPoint)
        self.free_variable = endPoint


class NestedProtocol(AbstractModel):
    """This type of model wraps the execution of an entire protocol."""

    def __init__(self, proto, inputExprs, output_names, optionalFlags):
        ...
        :param optionalFlags: list matching output_names specifying whether each output is optional (i.e. may be missing)
        """
    ...
    def get_outputs(self):
        ...
        for i, name in enumerate(self.output_names[:]):
            try:
                value = self.proto.outputEnv.look_up(name).unwrapped
            except Exception:
    ...
    def set_variable(self, name, valueExpr):
        self.proto.set_input(name, valueExpr)
...
class TestOdeModel(AbstractOdeModel):
    ...
    def __init__(self, a):
        ...
        self.output_names = ['a', 'y', 'state_variable', 'time',
